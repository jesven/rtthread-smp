...
#ifdef RT_USING_SMP
#define rt_thread_priority_table rt_cpu_self()->priority_table
#define rt_current_priority rt_cpu_self()->current_priority
#if RT_THREAD_PRIORITY_MAX > 32
/* Maximum priority level, 256 */

#define rt_thread_ready_priority_group rt_cpu_self()->priority_group
#define rt_thread_ready_table rt_cpu_self()->ready_table
#else
...
void rt_system_scheduler_init(void)
{
    ...
    for (cpu = 0; cpu < RT_CPUS_NR; cpu++)
    {
        for (offset = 0; offset < RT_THREAD_PRIORITY_MAX; offset ++)
        {
            rt_list_init(&rt_cpus[cpu].priority_table[offset]);
        }

        rt_cpus[cpu].current_priority = RT_THREAD_PRIORITY_MAX - 1;
        rt_cpus[cpu].current_thread = RT_NULL;
        rt_cpus[cpu].priority_group = 0;

#if RT_THREAD_PRIORITY_MAX > 32
        rt_memset(rt_cpus[cpu].ready_table, 0, sizeof(rt_cpus[cpu].ready_table));
#endif
    }
#endif /*RT_USING_SMP*/
...
void rt_system_scheduler_start(void)
{
    ...
    local_highest_ready_priority = (number << 3) + __rt_ffs(rt_thread_ready_table[number]) - 1;
#else
    highest_ready_priority = __rt_ffs(rt_global_thread_ready_priority_group) - 1;
    local_highest_ready_priority = __rt_ffs(rt_thread_ready_priority_group) - 1;
#endif
    ...
    {
        /* get switch to thread */
        to_thread = rt_list_entry(rt_thread_priority_table[local_highest_ready_priority].next,
                                  struct rt_thread,
                                  tlist);
    }
...
void rt_schedule(void)
{
    ...
    if (rt_current_thread->scheduler_lock_nest == 1 && rt_interrupt_nest == 0)
    {
        ...
        if (rt_global_thread_ready_priority_group != 0 || rt_thread_ready_priority_group != 0)
        {
            ...
            {
                _rt_schedule_insert_thread_no_ipi(rt_current_thread);
            }
...
static void _rt_schedule_insert_thread(struct rt_thread *thread, int send_ipi)
{
    ...
    {
        rt_list_insert_before(&(rt_cpus[thread->bind_cpu].priority_table[thread->current_priority]),
                              &(thread->tlist));
        if (send_ipi)
        {
            cpu_id = rt_cpuid();
    ...
    {
#if RT_THREAD_PRIORITY_MAX > 32
        rt_cpus[thread->bind_cpu].ready_table[thread->number] |= thread->high_mask;
#endif
        rt_cpus[thread->bind_cpu].priority_group |= thread->number_mask;
    }
...
void rt_schedule_remove_thread(struct rt_thread *thread)
{
    ...
    {
        if (rt_list_isempty(&(rt_cpus[thread->bind_cpu].priority_table[thread->current_priority])))
        {
#if RT_THREAD_PRIORITY_MAX > 32
            rt_cpus[thread->bind_cpu].ready_table[thread->number] &= ~thread->high_mask;
            if (rt_global_thread_ready_table[thread->number] == 0)
